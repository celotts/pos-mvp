...
# Puedes forzar uno específico ejecutando, por ejemplo: make up COMPOSE_CMD=podman-compose
...
.PHONY: help up down start logs ps clean shell lint format test
...
clean: down
	...
	# Limpia artefactos de Docker si existe
	@if command -v docker &> /dev/null; then \
		echo "Limpiando artefactos de Docker..."; \
		docker system prune -af; \
	fi
	# Limpia artefactos de Podman si existe
